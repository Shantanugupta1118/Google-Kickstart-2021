...
lambda_fnc_input = lambda: [*map(int, input().split())]
...
def connections(rows, cols):
    for i, row in rows.items():
        for x in row:
            if x[1] - x[0] < 2: continue
            for j in range(*x):
                vertex = next(vertex for vertex in cols[j] if vertex[0]<=i<vertex[1])
                if vertex[1] - vertex[0] < 2: continue
